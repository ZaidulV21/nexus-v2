...
import { useMyInvoice } from '@/queries/useInvoices';
      ...
      <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {/* Placeholder location for a future online payment action - the
                backend has no payment gateway in V1 (manual recording only). */}
          </CardContent>
        </Card>
      </div>
    ...
    <ul className="divide-y divide-border rounded-lg border border-border">
      {invoice.payments.map((payment) => (
        <li key={payment.id} className="flex flex-col gap-2 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <span className="text-sm text-ink-muted">{formatDateTime(payment.paidAt)}</span>
        </li>
      ))}
    </ul>
  );
