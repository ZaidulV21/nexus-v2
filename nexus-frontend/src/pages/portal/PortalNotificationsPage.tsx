...
import { Inbox, Info, AlertTriangle, CheckCircle, AlertCircle } from 'lucide-react';
...
function PortalNotificationRow({
  notification,
  onRead,
}: {
...
export function PortalNotificationsPage() {
  ...
  function handleNotificationClick(id: string, actionUrl?: string | null) {
    ...
    if (actionUrl) {
      const url = actionUrl.startsWith('/portal') ? actionUrl : `/portal${actionUrl}`;
      navigate(url);
    }
  }
  ...
  return (
    <div>
      <PageHeader
        title="Notifications"
        description="Stay updated on your projects and invoices"
      />
